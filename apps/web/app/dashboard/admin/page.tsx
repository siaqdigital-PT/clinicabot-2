...
interface FormState {
  ...
  phone: string;
  address: string;
  plan: "PILOT" | "STARTER" | "CLINIC" | "ENTERPRISE";
  primaryColor: string;
}
...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Se /api/admin/stats retornar 403, o utilizador não é SUPER_ADMIN — redirecionar
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Gestão global de todas as clínicas ClinicaBot</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {filteredClinics.map((clinic) => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${PLAN_COLORS[clinic.plan] ?? "bg-gray-100 text-gray-600"}`}
                    >
                      {clinic.plan}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    ...
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${clinic.isActive ? "bg-emerald-500" : "bg-red-500"}`}
                      />
      ...
      <Dialog.Root open={dialogOpen} onOpenChange={handleDialogClose}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-xl focus:outline-none">
            {created ? (
              /* ── Ecrã de confirmação com credenciais ── */
              <div className="space-y-4">
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
                  <p className="text-sm font-semibold text-amber-800">
                    Credenciais de acesso — guarda antes de fechar
                  </p>
                  ...
                  <p className="text-xs text-amber-700">
                    O admin deverá alterar a password no primeiro acesso.
                  </p>
                </div>
              ...
              <>
                ...
                <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
                  ...
                  {/* Slug */}
                  <div className="space-y-1.5">
                    <Label.Root htmlFor="c-slug" className="text-sm font-medium text-gray-700">
                      Slug (URL do widget)
                    </Label.Root>
                    <div className="flex items-center rounded-lg border border-gray-300 focus-within:border-[#1D9E75] focus-within:ring-1 focus-within:ring-[#1D9E75]">
                      <span className="select-none border-r border-gray-200 px-3 py-2 text-sm text-gray-400">
                        /widget/
                      </span>
                      <input
                        id="c-slug"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <Label.Root htmlFor="c-phone" className="text-sm font-medium text-gray-700">
                        Telefone
                      </Label.Root>
                      <input
                        id="c-phone"
                  ...
                  {/* Morada */}
                  <div className="space-y-1.5">
                    <Label.Root htmlFor="c-address" className="text-sm font-medium text-gray-700">
                      Morada
                    </Label.Root>
                    <input
                      id="c-address"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label.Root htmlFor="c-plan" className="text-sm font-medium text-gray-700">
                        Plano
                      </Label.Root>
                      <select
                        id="c-plan"
                        ...
                        className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-[#1D9E75] focus:ring-1 focus:ring-[#1D9E75]"
                      >
                        <option value="PILOT">PILOT — Gratuito</option>
                        <option value="STARTER">STARTER — €149/mês</option>
                        <option value="CLINIC">CLINIC — €299/mês</option>
                        <option value="ENTERPRISE">ENTERPRISE — €499/mês</option>
                      </select>
                    </div>
                    <div className="space-y-1.5">
                      <Label.Root htmlFor="c-color" className="text-sm font-medium text-gray-700">
                        Cor principal
                      </Label.Root>
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className={`mb-3 inline-flex rounded-lg p-2 ${c.bg} ${c.text}`}>{icon}</div>
...
function CredentialRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span
        className={`rounded bg-white px-2 py-0.5 text-xs font-semibold text-gray-800 ${mono ? "font-mono tracking-wider" : ""}`}
      >
        {value}
      </span>
    </div>
...
function BuildingIcon({ className }: { className?: string }) {
  return (
    <svg className={className ?? "h-5 w-5"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15l.75 18H3.75L4.5 3zM9 9h.008v.008H9V9zm0 3h.008v.008H9V12zm0 3h.008v.008H9V15zm4.5-6H13.5v.008H13.5V9zm0 3H13.5v.008H13.5V12zm0 3H13.5v.008H13.5V15z" />
    </svg>
  );
}
